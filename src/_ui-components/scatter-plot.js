 ...
 * 
 * Creates an interactive scatter plot visualization for UMAP data.
 * Features include zooming, tooltips, and interactive point selection.
 */

// Create UMAP scatter plot
function createUmapScatterPlot(containerId, data = [], options = {}) {
  console.log(`Creating UMAP scatter plot for container ${containerId} with ${data.length} data points`);
  ...
  if (!data || data.length === 0) {
    ...
    return {
      updateData: function(newData) {
        console.log("Updating scatter plot with new data");
        data = newData;
        updatePoints();
      }
    };
  ...
  return {
    updateData: function(newData) {
      console.log(`Updating scatter plot with ${newData.length} data points`);
      data = newData;
      updateScales(data);
      updatePoints();
    },
