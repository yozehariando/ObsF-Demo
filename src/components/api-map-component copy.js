...
function createApiMap(containerId, data, options = {}) {
  ...
  function updateLegend(radiusScale, maxCount) {
    ...
    const legendValues = [1, 5, 10]
    if (maxCount > 20) legendValues.push(Math.round(maxCount / 2))
    if (maxCount > 10) legendValues.push(maxCount)

    // Sort and deduplicate values
    const uniqueValues = [...new Set(legendValues)].sort((a, b) => a - b)

    // Calculate positions - use 40% of the width instead of full width
    const legendWidth =
      (innerWidth - config.legendMargin.left - config.legendMargin.right) * 0.6
    const startX = (innerWidth - legendWidth) / 2 // Center the legend horizontally
    const circleSpacing = legendWidth / (uniqueValues.length - 1) // Reduced spacing
    ...
    uniqueValues.forEach((value, i) => {
      ...
      legendGroup
        .append('circle')
        .attr('cx', cx)
        .attr('cy', 20) // Adjusted vertical position
        .attr('r', radius)
        .style('fill', '#6c757d')
      ...
      legendGroup
        .append('text')
        .attr('x', cx)
        .attr('y', 20 + radius + 15) // More space below circles
