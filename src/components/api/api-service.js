...
async function getSimilarSequences(jobId, options = {}) {
  try {
    ...
    return similarSequences.map(seq => {
      // Parse lat/lon from the metadata
      let latitude = null, longitude = null;
      if (seq.metadata?.lat_lon && seq.metadata.lat_lon[0]) {
        const [lat, lon] = seq.metadata.lat_lon[0].split(',').map(Number);
        latitude = lat;
        longitude = lon;
      }
      ...
      return {
        ...
        coordinates: [longitude, latitude], // For map
        umapCoordinates: coordinates, // For UMAP plot
