...
async function fetchAPIData() {
  // Show loading state
  const apiButton = document.querySelector("button.btn-primary");
  if (apiButton) {
  ...
  try {
    ...
    // Process the data to match our expected format if needed
    // This assumes the API returns data in the format we expect
    // If not, we'll need to transform it here
    return data;
  } catch (error) {
